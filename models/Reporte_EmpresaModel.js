...
ReporteEmpresaModel.findById = function (id,callback) {
    if (connection){
        ...
        var sql = "select reporte.*,reporte_empresa.*,usuario.nombre,usuario.idrol,categoria.*,empresa.nombre_empresa from reporte inner join reporte_empresa on reporte.idreporte = reporte_empresa.idreporte_empresa INNER JOIN usuario on reporte.idoperador = usuario.idusuario INNER JOIN categoria on categoria.idcategoria = reporte.idcategoria inner join empresa on reporte_empresa.idempresa= empresa.idempresa where reporte.idreporte = ?";
...
/*ReporteEmpresaModel.delete = function (id,callback) {
 ...
 ReporteEmpresaModel.estado = function(estado,callback){
    if (connection){
...
ReporteEmpresaModel.prio = function(estado,callback){
    if (connection){
...
ReporteEmpresaModel.cat = function(estado,callback){
    if (connection){
...
ReporteEmpresaModel.oper = function(estado,callback){
    if (connection){
